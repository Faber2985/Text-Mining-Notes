...
The course is quite nice and well structured, and has a lot of practical examples. But it goes quite fast,
...
Since we are going to compare corpora of text, we need a second set of tweets, and, following the example of the course,
...
coffee_corpus[[227]][1]
```
So we have removed special characters, punctuation and so on. Not all the words make much sense really (for example twitter usernames) but it should
not be a problem since we don't expect to see them very often in our corpus.
...
Note that the following command don't work from Rstudio if you want to use `knitr`. So the solution is to do 
...
We need of course first to do a cleanup of the bigrams list. But that is something that goes beyond the notes I am writing.
